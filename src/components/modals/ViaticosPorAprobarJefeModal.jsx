...
const ViaticosPorAprobarJefeModal = ({ isOpen, onClose, solicitud }) => {
  ...
  const totalPresupuesto = detallesPresupuesto.reduce((total, detalle) => {
    const precioUnitario = parseFloat(detalle.PrecioUnitario) || 0;  // Convertir a número flotante o 0
    const personas = parseFloat(detalle.Personas) || 1;              // Asegurarse de que sea al menos 1
    const cantidad = parseFloat(detalle.Cantidad) || 1;              // Asegurarse de que sea al menos 1
    return total + (precioUnitario * personas * cantidad);
  }, 0);
  ...
  return (
    ...
    <div className="w-full max-w-4xl max-h-[90vh] bg-white p-5 rounded-lg shadow-lg relative overflow-auto">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold">Viático por aprobar</h2>
        ...
        <table className="min-w-full divide-y divide-gray-200 mt-4">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {detallesPresupuesto.map((detalle, index) => {
              ...
              const personas = parseFloat(detalle.Personas) || 1;
              const cantidad = parseFloat(detalle.Cantidad) || 1;
              const total = precioUnitario * personas * cantidad;

              return (
                <tr key={index}>
                  <td className="py-2 px-4 text-sm">{detalle.Resumen}</td>
                  <td className="py-2 px-4 text-sm text-center">S/.{precioUnitario.toFixed(2)}</td>
                  <td className="py-2 px-4 text-sm text-center">{personas}</td>
                  <td className="py-2 px-4 text-sm text-center">{cantidad}</td>
                  <td className="py-2 px-4 text-sm text-center">S/.{total.toFixed(2)}</td>
                </tr>
              );
            })}
