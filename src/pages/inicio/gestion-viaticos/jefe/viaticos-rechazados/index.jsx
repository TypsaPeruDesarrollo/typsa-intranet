...
export default function ViaticosRechazados() {
  ...
  const fetchViaticos = async (empleadoId) => {
    try {
      ...
      if (res.ok) {
        ...
        const filteredData = data.filter(viatico => viatico.EstadoId === 4);
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="h-40 flex flex-col">
        <div className="h-40 bg-hero-pattern bg-center bg-cover w-full flex-1 relative">
          ...
          <div>
            <h1 className="absolute text-white text-4xl z-50 left-5 sm:left-10 lg:left-20 top-16 font-bold">Viáticos rechazados</h1>
          </div>
        </div>
      </div>
